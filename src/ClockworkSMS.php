...
use NotificationChannels\ClockworkSMS\Exceptions\ClockworkSMSException;
...
class ClockworkSMS
{
  ...
  */
  ...
  {
      if (empty($key)) {
          throw new ClockworkSMSException("Key can't be blank");
      } else {
          $this->key = $key;
      }
  ...
  {
      if (! is_array($sms)) {
          throw new ClockworkSMSException('sms parameter must be an array');
      }
      ...
      if (isset($err_no)) {
          throw new ClockworkSMSException($err_desc, $err_no);
      }
   ...
   *
   * @return  int   SMS credits remaining
